              ...
              <tr key={row.id} className="hover:bg-neutral-300/35">
                {row.getVisibleCells().map((cell, index) => {
                  return index === 0 ? (
                    ...
                    <td
                      key={cell.id}
                      className="box-border border-collapse bg-background py-3 pl-6 pr-4 text-[0.813rem] text-foreground/85 shadow-[inset_0_-1px_0_lightgray] md:first:sticky md:first:left-0 md:first:z-10 md:first:shadow-[inset_-1px_-1px_0_lightgray]"
                    >
                      {flexRender(
                        cell.column.columnDef.cell,
                        cell.getContext(),
