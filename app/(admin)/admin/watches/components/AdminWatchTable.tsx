...
const AdminWatchTable: React.FC<WatchTableProps> = ({ watches }) => {
  const router = useRouter();

  const data: Product[] = useMemo(() => watches, [watches]);
  ...
  const columns = [
    ...
    columnHelper.accessor((row) => row.caseType_ja, {
      id: "caseType_ja",
      cell: (info) => <span>{info.getValue()}</span>,
      header: () => <span>ケースタイプ</span>,
    }),
      ...
      cell: (info) => (
        <p>
          {info.getValue() > 0 ? (
            ...
            <span className="text-red">なし</span>
          )}
        </p>
      ),
      header: () => <span>在庫</span>,
        ...
        <Menu as={"div"} className={""}>
          ...
          >
            ...
            >
              <Menu.Item>
                <Link
                  href={`/admin/watches/${info.getValue()}`}
              ...
              <Menu.Item>
                <Link
                  href={`/admin/watches/edit/${info.getValue()}`}
                  className="flex items-center gap-3 px-3 py-3 text-xs"
                >
                  <FiEdit size={16} />
                  編集
...
export default AdminWatchTable;
