...
const caseColorSchema = z.object({
...
export async function POST(req: Request) {
  try {
    ...
    const result = caseColorSchema.safeParse(body);
    let zodErrors = {};
    ...
    const existingCaseColor = await prisma.caseColor.findFirst({
      where: {
        OR: [
    ...
    if (existingCaseColor) {
      return NextResponse.json(
        { message: "すでにこのケースの色は存在しています" },
        { status: 409 }
      );
    }

    const caseColor = await prisma.caseColor.create({
      data: {
        value: result.data.value,
        name_ja: result.data.name_ja,
        hexValue: result.data.hexValue,
    ...
    return NextResponse.json(caseColor, { status: 201 });
  } catch (error) {
    return NextResponse.json(
      { message: "エラーが発生しました" },
      { status: 500 }
