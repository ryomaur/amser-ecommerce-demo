...
製品が組み立てられていく様子をスクロールと連動し再生するページを追加しました。当初は動画ファイルをスクロールでプレイバックする仕様にするつもりでしたが、ブラウザーによってはフレームレートが落ちたり、フレームレートを高くエンコードするとファイルサイズが重くなってしまうなどの問題が発生したので、動画の各フレームを画像に変換してHTMLのcanvasに表示させる方式を取りました。結果的にページのロードが早くなり（useMemoも使用）パフォーマンスが向上しました。
